...
def get_similarities_common_neighbours(neighbours):
    """Returns the similarities for each node of every neighboring node 
       given the common neighbors metric"""
    p_dictionary = {}
    for node_i in neighbours:
        prob = []
        neighbours_i = set(neighbours[node_i])
        for node_j in neighbours[node_i]:
        ...
        p_dictionary[node_i] = (neighbours[node_i],prob)
...
def get_embeddings(input_file, output_folder, directed=False, walks_per_node=10, steps=80,
    ...
    """
    Performs non-uniform random walks (on all nodes) on given graph and generates its embeddings.
